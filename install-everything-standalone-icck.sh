...
## Intel Compiler Toolchain version
...
sed -i "s/+make clean/+CC=icc CXX=icpc FC=ifort HOSTCC=icc HOSTCXX=icpc HOSTFC=ifort CFLAGS=\"-Ofast -pipe -xHost -O3\" CXXFLAGS=\"-Ofast -pipe -xHost -O3\" make clean/g" ./0013-bootsplash.patch
sed -i "s/+make bootsplash-packer/+CC=icc CXX=icpc FC=ifort HOSTCC=icc HOSTCXX=icpc HOSTFC=ifort CFLAGS=\"-Ofast -pipe -xHost -O3\" CXXFLAGS=\"-Ofast -pipe -xHost -O3\" make bootsplash-packer/g" ./0013-bootsplash.patch
sed -i "s/  make prepare/  CC=icc CXX=icpc FC=ifort HOSTCC=icc HOSTCXX=icpc HOSTFC=ifort CFLAGS=\"-Ofast -pipe -xHost -O3\" CXXFLAGS=\"-Ofast -pipe -xHost -O3\" make prepare/g" ./PKGBUILD
sed -i "s/  yes \"\" | make config >\/dev\/null/  yes \"\" | CC=icc CXX=icpc FC=ifort HOSTCC=icc HOSTCXX=icpc HOSTFC=ifort CFLAGS=\"-Ofast -pipe -xHost -O3\" CXXFLAGS=\"-Ofast -pipe -xHost -O3\" make config >\/dev\/null/g" ./PKGBUILD
sed -i "s/  make \${MAKEFLAGS} LOCALVERSION= bzImage modules/  CC=icc CXX=icpc FC=ifort HOSTCC=icc HOSTCXX=icpc HOSTFC=ifort CFLAGS=\"-Ofast -pipe -xHost -O3\" CXXFLAGS=\"-Ofast -pipe -xHost -O3\" make \${MAKEFLAGS} LOCALVERSION= bzImage modules/g" ./PKGBUILD
sed -i "s/  _kernver=\"\$(make LOCALVERSION= kernelrelease)\"/  _kernver=\"\$(CC=icc CXX=icpc FC=ifort HOSTCC=icc HOSTCXX=icpc HOSTFC=ifort CFLAGS=\"-Ofast -pipe -xHost -O3\" CXXFLAGS=\"-Ofast -pipe -xHost -O3\" make LOCALVERSION= kernelrelease)\"/g" ./PKGBUILD
sed -i "s/  make LOCALVERSION= INSTALL_MOD_PATH=\"\${pkgdir}\/usr\" modules_install/  CC=icc CXX=icpc FC=ifort HOSTCC=icc HOSTCXX=icpc HOSTFC=ifort CFLAGS=\"-Ofast -pipe -xHost -O3\" CXXFLAGS=\"-Ofast -pipe -xHost -O3\" make LOCALVERSION= INSTALL_MOD_PATH=\"\${pkgdir}\/usr\" modules_install/g" ./PKGBUILD
...
# Export some Intel-specific compiler flags
unset CC
unset CXX
unset FC
unset HOSTCC
...
export HOSTCXX=icpc
export HOSTFC=ifort
export CFLAGS="-Ofast -pipe -xHost -O3"
export CXXFLAGS="-Ofast -pipe -xHost -O3"
